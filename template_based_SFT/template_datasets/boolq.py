...
class BoolQ(BaseDataset):
    def get_prompt_dataset(self, dataset, debug=False, mask_prompt=True):
        ...
        def generate_prompt(examples):
            sent1 = examples[columns[0]] # passage
            sent2 = examples[columns[1]] # question
            labels_origin = examples[columns[2]] # answer

            content_template = lambda x, y: f"{x.strip()}\nQuestion: {y.strip()}?"
    ...
    def __init__(self, tokenizer, max_len=64, debug=False, batch_size=4, num_workers=0):
        ...
        self.ids_to_labels = {0 : "False", 1 : "True"}
        self.prefix = 'Read the document below and answer the following question true or false.\n'
        self.suffix = '\nAnswer:'
        self.template = lambda o: f'{self.prefix}{o.strip()}{self.suffix}'
        self.columns = ['passage', 'question', 'label']
