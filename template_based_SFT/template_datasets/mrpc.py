...
class MRPC(BaseDataset):
    def get_prompt_dataset(self, dataset, debug=False, mask_prompt=True):
        ...
        def generate_prompt(examples):
            ...
            content_template = lambda x, y: f"Text1: {x.strip()}\nText2: {y.strip()}"
    ...
    def __init__(self, tokenizer, max_len=64, debug=False, batch_size=4, num_workers=0):
        ...
        self.ids_to_labels = {0 : "not_equivalent", 1 : "equivalent"}
        self.prefix = 'Are Text1 and Text2 equal to each other or not? Classify as either equivalent or not_equivalent.\n'
        self.suffix = '\nAnswer:'
        self.template = lambda o: f'{self.prefix}{o.strip()}{self.suffix}'
        self.columns = ['sentence1', 'sentence2', 'label']
