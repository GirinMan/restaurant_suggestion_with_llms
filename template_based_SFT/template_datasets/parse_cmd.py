...
class CMD(BaseDataset):
    def get_prompt_dataset(self, dataset, debug=False, mask_prompt=False):
        ...
        def generate_prompt(examples):
            documents = examples[columns[0]] # cmd
            time = examples[columns[1]] # time
            loc = examples[columns[2]] # loc
            pref = examples[columns[3]] # pref

            labels_origin = [f"시간: {_time}\n지역: {_loc}\n요구사항: {_pref}" for _time, _loc, _pref in zip(time, loc, pref)]
    ...
    def __init__(self, tokenizer, max_len=64, debug=False, batch_size=4, num_workers=0):
        super().__init__(tokenizer, max_len, batch_size, num_workers, eval_split="test", test_split="test")
        self.prefix = "입력된 명령에서 시간, 지역, 요구사항을 추출하세요.\n정보를 찾을 수 없을 경우 '없음'으로 표시하세요.\n명령: "
        self.suffix = '\n'
        self.template = lambda o: f'{self.prefix}{o.strip()}{self.suffix}'
        self.columns = ['cmd','time','loc','pref']

        dataset = datasets.load_dataset('csv', data_files="/home/girinman/repos/restaurant_suggestion_with_llms/template_based_SFT/template_datasets/raw/parse_cmd.csv")
